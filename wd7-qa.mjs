...
const {slug, canonical, section} = result;
const html = fs.readFileSync(result.output_path || path.join(slug, 'index.html'), 'utf8');
...
if (/INTERNAL EDITORIAL APPENDIX|NOT FOR PUBLICATION|BEGIN READER ARTICLE|END READER ARTICLE|Canonical URL:|## Front matter|\[VIS-\d{3}\s*(?:[—–-]|:)/i.test(html + source)) errors.push('internal workflow material leaked');
...
if (!['fundamentals','grapes','regions','winemaking'].includes(section)) errors.push('invalid website section');
else if (!fs.readFileSync(`${section}/index.html`,'utf8').includes(`href="${result.canonical_path || `/${slug}/`}"`)) errors.push(`${section} hub link missing`);
