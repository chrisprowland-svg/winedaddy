export default {
  async fetch(request, env) {
    ...
    if (isArticle) {
      ...
      html = html.replace(
        /(<article\b[^>]*>)[\s\S]*?(<aside class="(?:geography-panel|entity-links|learning-path-panel|grape-path-panel)"[\s\S]*?<section class="highlights">|<section class="highlights">)/i,
        '$1$2'
      );
    }

    // Navigation is generated once during the deterministic site build.
    // The serving Worker leaves it intact so preview and production cannot drift.
    html = html.replace(
